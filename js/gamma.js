...
var Gamma = (function() {
	...
	var $window = $( window ),
		...
		_initMasonry = function( callback ) {
			...
			Gamma.gallery.imagesLoaded( function() {
				...
				Gamma.gallery.masonry( {
					isAnimated: !Modernizr.csstransitions,  // Amit Tiwari update
					itemSelector : 'li',
		...
		add = function( $newitems, callback ) {
			...
			//Amit Tiwari update : wait for new images to be loaded
			$('.temp').remove();
			$('body').append("<div class='temp'></div>");
			$('.temp').css('width', 0).css('height',0).css('overflow','hidden');
			$('.temp').append($newitems.clone());
			$('.temp li > div').each(function(index, element) {
			   var smallImage = $(this).find('div').last();
               $(".temp").append("<img src='" + smallImage.attr('data-src') + "'></img>"); 
			   var mediumImage = smallImage.prev();
			   $(".temp").append("<img src='" + mediumImage.attr('data-src') + "'></img>"); 
            });
